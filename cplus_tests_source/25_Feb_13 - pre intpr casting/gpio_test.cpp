...
cout << "updating pGPIO to point to correct location for pin\n";
cout <<"pGPIO is pointing to = " << pGPIO << "\n";
...
uintptr_t testptr;
testptr = reinterpret_cast<uintptr_t>(pGPIO);
testptr = testptr & m;
//*pGPIO = *((reinterpret_cast<unsigned int*>(*pGPIO) & m));
pGPIO = reinterpret_cast<void*>(testptr);
...
//change pGPIO to point to correct location for pin (**not needed if input already performed as pGPIO location will be updated**)
//pGPIO = ((unsigned int*)pGPIO +((pin/10)*4));
